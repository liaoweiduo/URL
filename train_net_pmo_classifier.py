...
def train():
    ...
    with tf.compat.v1.Session(config=config) as session:
        ...
        '''load class mapping'''
        class_mapping_dict = json.load(open(os.path.join(args['model.dir'], 'summary', 'class_mapping.json'), 'r'))
        class_mapping = class_mapping_dict['classes_in_cluster']
        # {C0-C7: [[gt_label, label_str, domain_str, re_label],...]}
        num_train_classes = {cluster_name: len(items) for cluster_name, items in class_mapping.items()}
        print(f'num_train_classes: {num_train_classes}')
        # num_train_classes: e.g., {'C0': 3093, 'C1': 0, 'C2': 70, 'C3': 23, 'C4': 23, 'C5': 2323, 'C6': 23, 'C7':23}

        '''generate mapping from domain to clusters'''
        ...
        cluster_names = [f'C{idx}' for idx in range(args['model.num_clusters'])]        # f'C{idx}'
        # M0-net - M7-net
        for m_indx in range(args['model.num_clusters']):        # 8
            ...
            num_class = num_train_classes[cluster_names[m_indx]] if num_train_classes[cluster_names[m_indx]] != 0 else None
            _model = get_model(num_class, model_args_with_name, multi_device_id=m_indx, freeze_fe=False)
            # distribute model to multi-devices
            ...
            if os.path.isfile(_checkpointer.best_ckpt):
                load_iter, best_val_loss, best_val_acc =\
            ...
            if args['train.lr_policy'] == "step":
                _lr_manager = UniformStepLR(_optimizer, args, start_iter)   # start_iter == 0
