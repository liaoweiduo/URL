    ...
    with tf.compat.v1.Session(config=config) as session:
        ...
        for i in tqdm(range(max_iter), ncols=100):
            ...
            if (
                ...
                '''cluster for task'''
                cluster_idx, grad_one, similarity, task_centroid, loss_rec = pool.cluster_for_task(
                ...
                weighted_loss_rec = args['train.recon_weight'] * loss_rec
                weighted_loss_rec.backward(retain_graph=True)
                task_loss.backward()
                ...
                epoch_loss['task_rec'].append(loss_rec.item())

                del context_features_list, target_features_list, grad_ones, task_centroid, loss_rec, weighted_loss_rec
            ...
            if args['train.loss_type'] == 'task':
                with torch.no_grad():
                    info = pool.cluster_and_assign_from_buffer(cluster_url, clusterer)
            else:
                info = pool.cluster_and_assign_from_buffer(cluster_url, clusterer)

                loss_rec = info['loss_rec']
                weighted_loss_rec = args['train.recon_weight'] * loss_rec
                weighted_loss_rec.backward(retain_graph=True)
                epoch_loss['task_rec'].append(loss_rec.item())
            ...
            if (i + 1) % args['train.summary_freq'] == 0:        # 5; 2 for DEBUG
                ...
                for dataset_name in trainsets:
                    if dataset_name in epoch_loss.keys() and len(epoch_loss[dataset_name]) > 0:
