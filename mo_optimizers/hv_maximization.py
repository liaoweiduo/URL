...
class HvMaximization(object):
    ...
    """
    def __init__(self, n_mo_sol, n_mo_obj, ref_point=None, dyn_ref_mul=1.5, obj_space_normalize=True):
        super(HvMaximization, self).__init__()
        self.name = 'hv_maximization'
        self.ref_point = np.array(ref_point) if ref_point is not None else np.array([0.0 for _ in range(n_mo_obj)])
        # if ref_point is None, then use dynamic reference point based on dyn_ref_mul.
        self.dyn_ref_mul = dyn_ref_mul
        self.n_mo_sol = n_mo_sol
        self.n_mo_obj = n_mo_obj
        self.obj_space_normalize = obj_space_normalize

    def compute_weights(self, mo_obj_val):      # should be [num_obj, num_sol]
        ...
        dyn_ref_point = self.dyn_ref_mul * np.max(mo_obj_val, axis=1)
